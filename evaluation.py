...
__all__ = ['get_precision', 'get_p_1', 'get_p_3', 'get_p_5', 'get_p_10',
           ...
           'get_inv_propensity', 'get_psp', 'get_psndcg',
           'get_psp_1', 'get_psp_3', 'get_psp_5', 'get_psp_10',
           'get_psndcg_1', 'get_psndcg_3', 'get_psndcg_5', 'get_psndcg_10',
           'get_contrib', 'get_contrib_1', 'get_contrib_3', 'get_contrib_5', 'get_contrib_10']
...
def get_precision(prediction: TPredict, targets: TTarget, mlb: TMlb = None, classes: TClass = None, top=5):
    ...
    return prediction.multiply(targets).sum() / (top * targets.shape[0]) * 100
...
def get_ndcg(prediction: TPredict, targets: TTarget, mlb: TMlb = None, classes: TClass = None, top=5):
    ...
    return np.average(dcg / log.cumsum()[np.minimum(targets.sum(axis=-1), top) - 1]) * 100
...
def get_contrib(prediction: TPredict, targets: TTarget, mlb: TMlb = None, classes: TClass = None, top=5):
    mlb = get_mlb(classes, mlb, targets)
    if not isinstance(targets, csr_matrix):
        targets = mlb.transform(targets)
    prediction = mlb.transform(prediction[:, :top])
    return prediction.multiply(targets).sum(axis=0)


get_contrib_1 = partial(get_contrib, top=1)
get_contrib_3 = partial(get_contrib, top=3)
get_contrib_5 = partial(get_contrib, top=5)
get_contrib_10 = partial(get_contrib, top=10)
